defmodule IslandsEngine.Rules do
  ...
  # Initialized state -> When we’re in :initialized, the only permissible action is adding the second
  # player. We’re going to focus on that one action and the transition it triggers,from :initialized to :players_set.
  def check(%Rules{state: :initialized} = rules, :add_player) do
    {:ok, %Rules{rules | state: :players_set}}
  end

  # State for players_set
